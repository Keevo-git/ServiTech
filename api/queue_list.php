...
try {
  ...
  foreach ($rows as $r) {
    ...
    $out[] = [
      ...
      "service_label" => $details["service_name_snapshot"] ?? ($details["service_label"] ?? null),
      "paper_size" => $details["paper_size_snapshot"] ?? ($details["paper_size"] ?? null),
      "quantity" => $details["quantity_snapshot"] ?? ($details["quantity"] ?? null),
      "color_option" => $details["color_option_snapshot"] ?? ($details["color_option"] ?? null),
      "package_label" => $details["package_snapshot"] ?? ($details["package_label"] ?? null),
      "lamination_type" => $details["lamination_type_snapshot"] ?? ($details["lamination_type"] ?? null),
      "device_type" => $details["device_snapshot"] ?? ($details["device_type"] ?? null),
      "repair_type" => $details["service_type_snapshot"] ?? ($details["repair_type"] ?? null),
      "installation_type" => $details["installation_type_snapshot"] ?? ($details["installation_type"] ?? null),
      "notes" => $details["customer_notes_snapshot"] ?? ($details["notes"] ?? null),
      ...
      "price_per_page" => $details["price_snapshot"] ?? ($details["price_per_page"] ?? null),
      "estimated_total" => $details["final_total_snapshot"] ?? ($details["estimated_total"] ?? null),
