...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'queues_permanently_hidden_by_fkey'
      AND conrelid = 'queues'::regclass
  ) THEN
    ALTER TABLE queues
      ADD CONSTRAINT queues_permanently_hidden_by_fkey
      FOREIGN KEY (permanently_hidden_by) REFERENCES users(id) ON DELETE SET NULL;
  END IF;
END
$$;
